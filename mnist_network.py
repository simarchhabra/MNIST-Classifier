...
class Network:
    ...
    def model(self, training_data, epochs, mini_batch_size, learning_rate, regularization_parameter, epsilon):
        """
        Handles logistics of creating the model and training data. 
        Hyperparameter checks are done. Epochs and mini_batch_size are 
        used for stochastic gradient descent. Learning_rate is a usual
        hyperparameter and regularization_parameter done for L2 
        regularization. Epsilon is the name for early_stopping_threshold 
        value, done to optimize time and reduce overfitting.
        """
        
        print "Hyperparameters used - Hidden layers: {0} Epochs: {1} Mini_Batch_Size: {2} Learning Rate: {3} Regularization Parameter: {4} Early Stopping Threshold: {5}".format(self.sizes[1], 
                epochs, mini_batch_size, learning_rate, regularization_parameter, epsilon)
        ...
        for epoch in xrange(epochs):
            ...
            for batch in batches:
                count += self.update_network(batch, learning_rate, regularization_parameter, len(training_data))
    ...
    def update_network(self, mini_batch, learning_rate, regularization_parameter, n):
        ...
        for train_data in mini_batch:
            ...
            w_grad_sum = [cv+wg+((regularization_parameter*wg)/n) for cv,wg in zip(w_grad_sum, w_grad)] 
            count+=prediction
    ...
    def backpropagate(self, x, y):
        """
        Calculates gradient of weights and biases with respect to cost
    ...
    def predict(self, testing_data, regularization_parameter):
        """
        Handles predictive part of model. Uses testing data to determine
        classification accuracy. Calculates cost of model in relation to test
        data as an objective function to minimize for bayesian hyperparameter
        optimization script
        """
        count = 0
        cost = 0.0
        for test in testing_data:
            ...
            if self.evaluate(activations[-1], test_output):
                count+=1
            # regular gradient cost
            c_zero = self.get_cost(activations[-1], test_output)
            cost += c_zero
        # L2 regularization cost
        c_reg = (regularization_parameter/(2*len(testing_data))) * sum(map(lambda w:np.sum(w**2), self.weights))
        cost += c_reg
